...
<div class="admin-shell flex items-center justify-center px-4 py-16 font-sans">
	<div class="glass-panel w-full max-w-md p-8 md:p-10">
		<h1 class="text-2xl font-bold tracking-tight text-text-main">Forgot password</h1>
		<p class="mt-1 mb-6 text-sm text-text-muted">Enter your admin account email and we'll send you a reset link.</p>

		{#if form?.success}
			<div class="success-banner">
				If that email has an account, a reset link is on its way. <a href="/admin" class="underline">Back to sign in</a>.
			</div>
		{:else}
			{#if form?.error}
