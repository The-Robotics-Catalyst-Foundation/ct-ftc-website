<script>
  ...
  onMount(() => {
    ...
    userEmail = pb.authStore.model?.email || "Administrator";
  });
...
<aside class="w-full md:w-64 bg-white border-b-4 md:border-b-0 md:border-r-4 border-black p-6 flex flex-col justify-between md:min-h-screen shrink-0 z-30 relative">
  <div class="space-y-8">
    ...
    <nav aria-label="Admin Navigation" class="flex flex-row md:flex-col gap-2 overflow-x-auto md:overflow-visible pb-2 md:pb-0">
      <a 
        href="/admin" 
  ...
  <div class="mt-6 md:mt-auto pt-4 border-t-2 border-black/10 space-y-4 text-left">
    <div class="truncate">
      <span class="block text-[9px] font-mono font-black uppercase text-slate-400 tracking-widest">Logged In As</span>
      <span class="text-xs font-black text-black block truncate" title={userEmail}>{userEmail}</span>
    </div>
